...
public class CustomListView extends ListView {
	...
	private Handler mHandler = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			switch (msg.what) {
			case SINGLE_TAP:
				Logger.i(TAG, "Single tap entry");
				mOnDoubleTapListener
						.OnSingleTap(mParent, mView, mPosition, mId);
				break;
			case DOUBLE_TAP:
				Logger.i(TAG, "Double tap entry");
				mOnDoubleTapListener
						.OnDoubleTap(mParent, mView, mPosition, mId);
				break;
			}
		}
	...
	public void setOnItemDoubleClickListener(OnItemDoubleTapLister listener) {
		...
		if (mOnDoubleTapListener == null)
			throw new IllegalArgumentException(
					"OnItemDoubleTapListener cannot be null");
		else {
			/*
			 * If the OnItemDoubleTapListener is not null, register the default
			 * onItemClickListener to proceed with listening
			 */
			setOnItemClickListener(new OnItemClickListener() {
				...
				public void onItemClick(AdapterView<?> parent, View view,
					...
					if (!mTookFirstEvent) /* Testing if first tap occurred */
					{
						 ...
						 * event. In case user presses any other item (position)
						 */
						mTookFirstEvent = true;
						mMessage = mMessage == null ? new Message() : mHandler
								.obtainMessage();
						/*
						...
						if (mPositionHolder == position) {
							 ...
							 * scheduled sending after elapsed time > DELAY,
							 * immediately when we have second event, when the
							 * time is < DELAY
							 */
							mPosition = position;
							 ...
							 * second event, when the time is < DELAY
							 */
